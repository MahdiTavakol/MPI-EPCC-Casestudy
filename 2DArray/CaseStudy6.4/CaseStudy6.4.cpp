...
int main(int argc, char** argv)
{
    ...
    masterbuff = allocate(masterbuff,M+2,N+2);
    new_ptr    = allocate(new_ptr,Mp+2,Np+2);
    old        = allocate(old,Mp+2,Np+2);
    
    edge = (double**) malloc((Mp+2)*sizeof(double*));
    ...
    float start = MPI_Wtime();
    {
        ...
        MPI_Bcast(masterbuff[0], (M+2)*(N+2), MPI_DOUBLE, 0, world);
        for (int i = 0; i < Mp+2; i++)
            edge[i] = masterbuff[rank*Mp+i];
        ...
        MPI_Gather(old[0], (Mp+2)*(Np+2), MPI_DOUBLE,masterbuff[0],(Mp+2)*(Np+2),MPI_DOUBLE,0,world);
        if (rank == 0)
        {
    ...
    deallocate(old);
    free(edge);
    // We must not deallocate edge[0] since it points to a location in
    // masterbuff which has been deallocated. So, deallocating it leads
